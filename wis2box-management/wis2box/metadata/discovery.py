...
class DiscoveryMetadata(BaseMetadata):
    ...
    def get_distribution_links(self,
        ...
        canonical_link = {
            'href': f"{URL}/data/metadata/{identifier}.json",  # noqa
...
def publish_discovery_metadata(metadata: Union[dict, str]):
    ...
    try:
        ...
        if len(err.errors) == 2 and 'message' in err.errors[0] and err.errors[0]['message'].startswith('Invalid centre_id'):  # noqa
